...
do {
    ...
    if (isset($Data['response']['score'])) {
        if (!isset($Data['response']['clan_info']['accountid'])) {
            ...
            if ($Data['response']['clan_info']['accountid'] != 4777282) {
...
do {
    ...
    do {
        ...
        if ($BestPlanetAndZone['id'] !== $SteamThinksPlanet) {
            SendPOST('ITerritoryControlMinigameService/JoinPlanet',
                'id=' . $BestPlanetAndZone['id'] . '&access_token=' . $Token);

            $SteamThinksPlanet = LeaveCurrentGame($Token);
        }
    } while ($BestPlanetAndZone['id'] !== $SteamThinksPlanet);

    $Zone = SendPOST('ITerritoryControlMinigameService/JoinZone',
        'zone_position=' . $BestPlanetAndZone['best_zone']['zone_position'] . '&access_token=' . $Token);
    ...
    if ($LagAdjustedWaitTime > 0) {
        Msg('   {grey}Waiting ' . number_format($LagAdjustedWaitTime,
                3) . ' remaining seconds before submitting score...');

        usleep($LagAdjustedWaitTime * 1000000);
    }

    $WaitedTimeAfterJoinZone = microtime(true) - $WaitedTimeAfterJoinZone;
    Msg('   {grey}Waited ' . number_format($WaitedTimeAfterJoinZone, 3) . ' (+' . number_format($SkippedLagTime,
            4) . ' lag) total seconds before sending score');

    $Data = SendPOST('ITerritoryControlMinigameService/ReportScore',
        'access_token=' . $Token . '&score=' . GetScoreForZone($Zone) . '&language=english');

    if ($Data['eresult'] == 93) {
        $LagAdjustedWaitTime = $SkippedLagTime + 0.3;

        Msg('{lightred}-- EResult 93 means time is out of sync, trying again in ' . number_format($LagAdjustedWaitTime,
                3) . ' seconds...');
...
function LeaveCurrentGame($Token, $LeaveCurrentPlanet = 0)
{
    do {
        ...
        if (isset($Data['response']['active_zone_game'])) {
            SendPOST('IMiniGameService/LeaveGame',
                'access_token=' . $Token . '&gameid=' . $Data['response']['active_zone_game']);
        }
    } while (!isset($Data['response']['score']));
